...
import type { AuthRequest, ApiKey } from '../types/index.js';
...
router.delete('/api-keys/:keyId', async (req: AuthRequest, res: Response) => {
  try {
    ...
    const success = apiKeysDb.deleteApiKey(req.user!.id, parseInt(keyId as string));
...
router.patch('/api-keys/:keyId/toggle', async (req: AuthRequest, res: Response) => {
  try {
    const { keyId } = req.params;
    ...
    const success = apiKeysDb.toggleApiKey(req.user!.id, parseInt(keyId as string), isActive);
...
// Delete a credential
router.delete('/credentials/:credentialId', async (req: AuthRequest, res: Response) => {
  try {
    const { credentialId } = req.params;
    const success = credentialsDb.deleteCredential(req.user!.id, parseInt(credentialId as string));
...
router.patch('/credentials/:credentialId/toggle', async (req: AuthRequest, res: Response) => {
  try {
    const { credentialId } = req.params;
    const { isActive } = req.body;
    ...
    const success = credentialsDb.toggleCredential(req.user!.id, parseInt(credentialId as string), isActive);
