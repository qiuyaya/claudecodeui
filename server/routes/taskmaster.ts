...
import type { MCPDetectionResult } from '../utils/mcp-detector.js';
import { broadcastTaskMasterProjectUpdate, broadcastTaskMasterTasksUpdate } from '../utils/taskmaster-websocket.js';
import type { TaskMasterTask } from '../types/index.js';
 ...
 * @returns {Promise<InstallationResult>} Installation status result
 */
async function checkTaskMasterInstallation(): Promise<InstallationResult> {
    return new Promise((resolve: (value: InstallationResult) => void) => {
        // Check if task-master command is available
        const child: ChildProcess = spawn('which', ['task-master'], {
        ...
        child.on('close', (code: number | null) => {
            if (code === 0 && output.trim()) {
                ...
                const versionChild: ChildProcess = spawn('task-master', ['--version'], {
                    stdio: ['ignore', 'pipe', 'pipe'],
                    shell: true
                });
                ...
                versionChild.stdout!.on('data', (data: Buffer) => {
                    versionOutput += data.toString();
                });

                versionChild.on('close', (versionCode: number | null) => {
                    resolve({
                        isInstalled: true,
        ...
        child.on('error', (error: Error) => {
            resolve({
                ...
                installPath: null,
                version: null,
                reason: `Error checking installation: ${(error as Error).message}`
            });
        });
    });
}
 ...
 * @returns {Promise<DetectionResult>} Detection result with status and metadata
 */
async function detectTaskMasterFolder(projectPath: string): Promise<DetectionResult> {
    try {
        const taskMasterPath: string = path.join(projectPath, '.taskmaster');

        // Check if .taskmaster directory exists
        try {
            const stats: fs.Stats = await fsPromises.stat(taskMasterPath);
        ...
        if (fileStatus['tasks/tasks.json']) {
            try {
                ...
                const tasksContent: string = await fsPromises.readFile(tasksPath, 'utf8');
                const tasksData = JSON.parse(tasksContent) as Record<string, unknown>;

                // Handle both tagged and legacy formats
                let tasks: RawTaskData[] = [];
                if ((tasksData as { tasks?: RawTaskData[] }).tasks) {
                    // Legacy format
                    tasks = (tasksData as { tasks: RawTaskData[] }).tasks;
                } else {
                    // Tagged format - get tasks from all tags
                    Object.values(tasksData).forEach((tagData: unknown) => {
                        const td = tagData as TasksFileTagContent;
                        if (td.tasks) {
                            tasks = tasks.concat(td.tasks);
                        }
                    });
                }

                // Calculate task statistics
                const stats: TaskStats = tasks.reduce((acc: TaskStats, task: RawTaskData) => {
                    acc.total++;
                    ...
                    // Count subtasks
                    if (task.subtasks) {
                        task.subtasks.forEach((subtask: RawTaskData) => {
                            acc.subtotalTasks++;
 ...
 */
router.get('/installation-status', async (req: Request, res: Response): Promise<void> => {
    try {
        const installationStatus: InstallationResult = await checkTaskMasterInstallation();

        // Also check for MCP server configuration
        const mcpStatus: MCPDetectionResult = await detectTaskMasterMCPServer();
        ...
        res.status(500).json({
            ...
            installation: {
                isInstalled: false,
                reason: `Server error: ${(error as Error).message}`
            },
            mcpServer: {
                hasMCPServer: false,
                reason: `Server error: ${(error as Error).message}`
 ...
 */
router.get('/detect/:projectName', async (req: Request, res: Response): Promise<void> => {
    try {
        ...
        const [taskMasterResult, mcpResult]: [DetectionResult, MCPDetectionResult] = await Promise.all([
            detectTaskMasterFolder(projectPath),
            detectTaskMasterMCPServer()
        ]);

        // Determine overall status
        let status: string = 'not-configured';
 ...
 */
router.get('/detect-all', async (req: Request, res: Response): Promise<void> => {
    try {
        ...
        const detectionPromises: Promise<ProjectDetectionResult>[] = projects.map(async (project: ProjectListItem) => {
            try {
                ...
                if (project.fullPath) {
                    ...
                    try {
                        ...
                        throw new Error(`Failed to extract project directory: ${(error as Error).message}`);
                    }
                }
        ...
        const results: ProjectDetectionResult[] = await Promise.all(detectionPromises);

        res.json({
            projects: results,
            summary: {
                total: results.length,
                fullyConfigured: results.filter((p: ProjectDetectionResult) => p.status === 'fully-configured').length,
                taskmasterOnly: results.filter((p: ProjectDetectionResult) => p.status === 'taskmaster-only').length,
                mcpOnly: results.filter((p: ProjectDetectionResult) => p.status === 'mcp-only').length,
                notConfigured: results.filter((p: ProjectDetectionResult) => p.status === 'not-configured').length,
                errors: results.filter((p: ProjectDetectionResult) => p.status === 'error').length
            },
            timestamp: new Date().toISOString()
        });
 ...
 */
router.post('/initialize/:projectName', async (req: Request, res: Response): Promise<void> => {
    try {
        const { projectName } = req.params;
 ...
 */
router.get('/next/:projectName', async (req: Request, res: Response): Promise<void> => {
    try {
        const { projectName } = req.params;
        ...
        try {
            ...
            const nextTaskCommand: ChildProcess = spawn('task-master', ['next'], {
                cwd: projectPath,
            ...
            nextTaskCommand.stdout!.on('data', (data: Buffer) => {
                stdout += data.toString();
            });

            nextTaskCommand.stderr!.on('data', (data: Buffer) => {
                stderr += data.toString();
            });

            await new Promise<void>((resolve: () => void, reject: (reason: Error) => void) => {
                nextTaskCommand.on('close', (code: number | null) => {
                    if (code === 0) {
                        resolve();
                    } else {
                ...
                nextTaskCommand.on('error', (error: Error) => {
                    reject(error);
                });
            });

            // Parse the output - task-master next usually returns JSON
            let nextTaskData: Record<string, unknown> | null = null;
            if (stdout.trim()) {
                try {
            ...
            console.warn('Failed to execute task-master CLI:', (cliError as Error).message);

            // Fallback to loading tasks and finding next one locally
            // Use localhost to bypass proxy for internal server-to-server calls
            const tasksResponse: globalThis.Response = await fetch(`http://localhost:${process.env.PORT || 3001}/api/taskmaster/tasks/${encodeURIComponent(projectName as string)}`, {
                headers: {
            ...
            if (tasksResponse.ok) {
                const tasksData = await tasksResponse.json() as { tasks?: TransformedTask[] };
                const nextTask: TransformedTask | null = tasksData.tasks?.find((task: TransformedTask) =>
 ...
 */
router.get('/tasks/:projectName', async (req: Request, res: Response): Promise<void> => {
    try {
        const { projectName } = req.params;
        ...
        const taskMasterPath: string = path.join(projectPath, '.taskmaster');
        const tasksFilePath: string = path.join(taskMasterPath, 'tasks', 'tasks.json');
        ...
        try {
            const tasksContent: string = await fsPromises.readFile(tasksFilePath, 'utf8');
            const tasksData = JSON.parse(tasksContent) as Record<string, unknown>;

            let tasks: RawTaskData[] = [];
            let currentTag: string = 'master';

            // Handle both tagged and legacy formats
            if (Array.isArray(tasksData)) {
                // Legacy format
                tasks = tasksData as RawTaskData[];
            } else if ((tasksData as { tasks?: RawTaskData[] }).tasks) {
                // Simple format with tasks array
                tasks = (tasksData as { tasks: RawTaskData[] }).tasks;
            } else {
                // Tagged format - get tasks from current tag or master
                const taggedData = tasksData as Record<string, TasksFileTagContent>;
                if (taggedData[currentTag] && taggedData[currentTag].tasks) {
                    tasks = taggedData[currentTag].tasks!;
                } else if (taggedData.master && taggedData.master.tasks) {
                    tasks = taggedData.master.tasks;
                } else {
                    // Get tasks from first available tag
                    const firstTag: string | undefined = Object.keys(tasksData).find((key: string) =>
                        (tasksData as Record<string, TasksFileTagContent>)[key].tasks && Array.isArray((tasksData as Record<string, TasksFileTagContent>)[key].tasks)
                    );
                    if (firstTag) {
                        tasks = taggedData[firstTag].tasks!;
                        currentTag = firstTag;
                    }
                }
            }
            ...
            const transformedTasks: TransformedTask[] = tasks.map((task: RawTaskData) => ({
                id: task.id,
            ...
            res.json({
                ...
                tasksByStatus: {
                    pending: transformedTasks.filter((t: TransformedTask) => t.status === 'pending').length,
                    'in-progress': transformedTasks.filter((t: TransformedTask) => t.status === 'in-progress').length,
                    done: transformedTasks.filter((t: TransformedTask) => t.status === 'done').length,
                    review: transformedTasks.filter((t: TransformedTask) => t.status === 'review').length,
                    deferred: transformedTasks.filter((t: TransformedTask) => t.status === 'deferred').length,
                    cancelled: transformedTasks.filter((t: TransformedTask) => t.status === 'cancelled').length
                },
                timestamp: new Date().toISOString()
            });
 ...
 */
router.get('/prd/:projectName', async (req: Request, res: Response): Promise<void> => {
    try {
        const { projectName } = req.params;
        ...
        const docsPath: string = path.join(projectPath, '.taskmaster', 'docs');
        ...
        try {
            const files: string[] = await fsPromises.readdir(docsPath);
            const prdFiles: PrdFileInfo[] = [];

            for (const file of files) {
                const filePath: string = path.join(docsPath, file);
                const stats: fs.Stats = await fsPromises.stat(filePath);
            ...
            res.json({
                projectName,
                projectPath,
                prdFiles: prdFiles.sort((a: PrdFileInfo, b: PrdFileInfo) => new Date(b.modified).getTime() - new Date(a.modified).getTime()),
                timestamp: new Date().toISOString()
            });
 ...
 */
router.post('/prd/:projectName', async (req: Request, res: Response): Promise<void> => {
    try {
        const { projectName } = req.params;
        const { fileName, content }: { fileName: string; content: string } = req.body;
        ...
        const docsPath: string = path.join(projectPath, '.taskmaster', 'docs');
        const filePath: string = path.join(docsPath, fileName);
 ...
 */
router.get('/prd/:projectName/:fileName', async (req: Request, res: Response): Promise<void> => {
    try {
        ...
        const filePath: string = path.join(projectPath, '.taskmaster', 'docs', fileName as string);
        ...
        // Read file content
        try {
            const content: string = await fsPromises.readFile(filePath, 'utf8');
            const stats: fs.Stats = await fsPromises.stat(filePath);
 ...
 */
router.delete('/prd/:projectName/:fileName', async (req: Request, res: Response): Promise<void> => {
    try {
        const { projectName, fileName } = req.params;
 ...
 */
router.post('/init/:projectName', async (req: Request, res: Response): Promise<void> => {
    try {
        const { projectName } = req.params;
        ...
        const initProcess: ChildProcess = spawn('npx', ['task-master', 'init'], {
            cwd: projectPath,
            stdio: ['pipe', 'pipe', 'pipe']
        });
 ...
 */
router.post('/add-task/:projectName', async (req: Request, res: Response): Promise<void> => {
    try {
        const { projectName } = req.params;
        const { prompt, title, description, priority = 'medium', dependencies }: {
            prompt?: string;
            title?: string;
            description?: string;
            priority?: string;
        ...
        const args: string[] = ['task-master-ai', 'add-task'];
        ...
        const addTaskProcess: ChildProcess = spawn('npx', args, {
            cwd: projectPath,
            stdio: ['pipe', 'pipe', 'pipe']
        });
        ...
        addTaskProcess.stdout!.on('data', (data: Buffer) => {
            stdout += data.toString();
        });

        addTaskProcess.stderr!.on('data', (data: Buffer) => {
            stderr += data.toString();
        });

        addTaskProcess.on('close', (code: number | null) => {
 ...
 */
router.put('/update-task/:projectName/:taskId', async (req: Request, res: Response): Promise<void> => {
    try {
        const { projectName, taskId } = req.params;
        const { title, description, status, priority, details }: {
            title?: string;
            description?: string;
            status?: string;
            priority?: string;
        ...
        if (status && Object.keys(req.body).length === 1) {
            const setStatusProcess: ChildProcess = spawn('npx', ['task-master-ai', 'set-status', `--id=${taskId}`, `--status=${status}`], {
                cwd: projectPath,
                stdio: ['pipe', 'pipe', 'pipe']
            });
            ...
            setStatusProcess.stdout!.on('data', (data: Buffer) => {
                stdout += data.toString();
            });

            setStatusProcess.stderr!.on('data', (data: Buffer) => {
                stderr += data.toString();
            });

            setStatusProcess.on('close', (code: number | null) => {
                if (code === 0) {
                    // Broadcast task update via WebSocket
                    if (req.app.locals.wss) {
                        broadcastTaskMasterTasksUpdate(req.app.locals.wss, projectName as string, null);
                    }
            ...
            const prompt: string = `Update task with the following changes: ${updates.join(', ')}`;

            const updateProcess: ChildProcess = spawn('npx', ['task-master-ai', 'update-task', `--id=${taskId}`, `--prompt=${prompt}`], {
                cwd: projectPath,
                stdio: ['pipe', 'pipe', 'pipe']
            });
 ...
 */
router.post('/parse-prd/:projectName', async (req: Request, res: Response): Promise<void> => {
    try {
        const { projectName } = req.params;
        const { fileName = 'prd.txt', numTasks, append = false }: {
            fileName?: string;
            numTasks?: number;
            append?: boolean;
        } = req.body;
        ...
        const prdPath: string = path.join(projectPath, '.taskmaster', 'docs', fileName);
        ...
        const args: string[] = ['task-master-ai', 'parse-prd', prdPath];
        ...
        const parsePRDProcess: ChildProcess = spawn('npx', args, {
            cwd: projectPath,
            stdio: ['pipe', 'pipe', 'pipe']
        });
        ...
        parsePRDProcess.stdout!.on('data', (data: Buffer) => {
            stdout += data.toString();
        });

        parsePRDProcess.stderr!.on('data', (data: Buffer) => {
            stderr += data.toString();
        });

        parsePRDProcess.on('close', (code: number | null) => {
            if (code === 0) {
                // Broadcast task update via WebSocket
                if (req.app.locals.wss) {
                    broadcastTaskMasterTasksUpdate(
 ...
 */
router.get('/prd-templates', async (req: Request, res: Response): Promise<void> => {
    try {
...
2. **Main Application Features**
   - As a user, I want [core feature 1] so I can [benefit]
   - As a user, I want [core feature 2] so I can [benefit]
   - As a user, I want [core feature 3] so I can [benefit]
 ...
 */
router.post('/apply-template/:projectName', async (req: Request, res: Response): Promise<void> => {
    try {
        const { projectName } = req.params;
        const { templateId, fileName = 'prd.txt', customizations = {} }: {
            templateId?: string;
            fileName?: string;
        ...
        const templates: PrdTemplate[] = await getAvailableTemplates();
        const template: PrdTemplate | undefined = templates.find((t: PrdTemplate) => t.id === templateId);
        ...
        const docsDir: string = path.join(projectPath, '.taskmaster', 'docs');
        try {
        ...
        const filePath: string = path.join(docsDir, fileName);
...
async function getAvailableTemplates(): Promise<PrdTemplate[]> {
