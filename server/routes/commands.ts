...
async function scanCommandsDirectory(dir: string, baseDir: string, namespace: string): Promise<Command[]> {
  ...
  try {
    ...
    for (const entry of entries) {
      ...
      if (entry.isDirectory()) {
        ...
        try {
          ...
          console.error(`Error parsing command file ${fullPath}:`, (err as Error).message);
        }
      }
    }
  } catch (err: unknown) {
    ...
    if ((err as NodeJS.ErrnoException).code !== 'ENOENT' && (err as NodeJS.ErrnoException).code !== 'EACCES') {
      console.error(`Error scanning directory ${dir}:`, (err as Error).message);
    }
  }
 ...
 * Each handler returns { type: 'builtin', action: string, data: Record<string, unknown> }
 */
const builtInHandlers: Record<string, (args: string[], context: CommandContext) => Promise<CommandResult>> = {
...
`;
  ...
  '/clear': async (args: string[], context: CommandContext) => {
    return {
      type: 'builtin',
      action: 'clear',
      data: {
  ...
  '/model': async (args: string[], context: CommandContext) => {
  ...
  '/cost': async (args: string[], context: CommandContext) => {
    ...
    const pricingByProvider: Record<string, { input: number; output: number }> = {
  ...
  '/rewind': async (args: string[], context: CommandContext) => {
...
router.post('/list', async (req: Request, res: Response) => {
  try {
    ...
    const allCommands: Command[] = [...builtInCommands];
