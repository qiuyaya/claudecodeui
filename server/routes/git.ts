...
async function validateGitRepository(projectPath: string): Promise<void> {
  ...
  try {
    ...
    if ((error as Error).message.includes('Project directory is not a git repository')) {
      throw error;
    }
...
router.get('/status', async (req: Request, res: Response) => {
  ...
  try {
    ...
    try {
      ...
      if ((error as Error).message.includes('unknown revision') || (error as Error).message.includes('ambiguous argument')) {
        hasCommits = false;
        branch = 'main';
      } else {
        throw error;
    ...
    res.json({
      error: errMsg.includes('not a git repository') || errMsg.includes('Project directory is not a git repository')
        ? errMsg
        : 'Git operation failed',
      details: errMsg.includes('not a git repository') || errMsg.includes('Project directory is not a git repository')
        ? errMsg
        : `Failed to get git status: ${errMsg}`
    });
  }
});
...
router.get('/file-with-diff', async (req: Request, res: Response) => {
  const { project, file } = req.query as { project?: string; file?: string };
...
router.post('/initial-commit', async (req: Request, res: Response) => {
...
router.get('/commits', async (req: Request, res: Response) => {
  const { project, limit = '10' } = req.query as { project?: string; limit?: string };
  ...
  try {
    ...
    const commits: Array<{ hash: string; author: string; email: string; date: string; message: string; stats?: string }> = stdout
      .split('\n')
...
router.get('/commit-diff', async (req: Request, res: Response) => {
  const { project, commit } = req.query as { project?: string; commit?: string };
...
router.post('/generate-commit-message', async (req: Request, res: Response) => {
 ...
 */
async function generateCommitMessageWithAI(files: string[], diffContext: string, provider: string, projectPath: string): Promise<string> {
  // Create the prompt
...
router.post('/fetch', async (req: Request, res: Response) => {
  ...
  try {
    ...
    res.status(500).json({
      error: 'Fetch failed',
      details: errMsg.includes('Could not resolve hostname')
        ? 'Unable to connect to remote repository. Check your internet connection.'
        : errMsg.includes("fatal: 'origin' does not appear to be a git repository")
...
router.post('/pull', async (req: Request, res: Response) => {
  ...
  try {
    ...
    } else if (errMsg.includes('Please commit your changes or stash them')) {
      errorMessage = 'Uncommitted changes detected';
      details = 'Please commit or stash your local changes before pulling.';
    } else if (errMsg.includes('Could not resolve hostname')) {
      errorMessage = 'Network error';
      details = 'Unable to connect to remote repository. Check your internet connection.';
    } else if (errMsg.includes("fatal: 'origin' does not appear to be a git repository")) {
      errorMessage = 'Remote not configured';
      details = 'No remote repository configured. Add a remote with: git remote add origin <url>';
    } else if (errMsg.includes('diverged')) {
...
router.post('/push', async (req: Request, res: Response) => {
  ...
  try {
    ...
    } else if (errMsg.includes('non-fast-forward')) {
    ...
    } else if (errMsg.includes('no upstream branch')) {
...
router.post('/publish', async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (errMsg.includes('rejected')) {
      ...
      errorMessage = 'Authentication failed';
      details = 'Permission denied. Check your credentials or SSH keys.';
    } else if (errMsg.includes('fatal:') && errMsg.includes('does not appear to be a git repository')) {
      errorMessage = 'Remote not configured';
...
router.post('/delete-untracked', async (req: Request, res: Response) => {
  const { project, file } = req.body;
