...
const c: Record<string, (text: string) => string> = {
...
const packageJson: { version: string; homepage?: string; bugs?: { url?: string } } = JSON.parse(fs.readFileSync(packageJsonPath, 'utf8'));
...
function showStatus(): void {
    ...
    const claudeProjectsPath: string = path.join(os.homedir(), '.claude', 'projects');
    const projectsExists: boolean = fs.existsSync(claudeProjectsPath);
    ...
    const envExists: boolean = fs.existsSync(envFilePath);
...
// Check for updates
async function checkForUpdates(silent: boolean = false): Promise<{ hasUpdate: boolean; latestVersion?: string; currentVersion?: string; error?: string }> {
    try {
        const { execSync } = await import('child_process');
        const latestVersion: string = execSync('npm show @siteboon/claude-code-ui version', { encoding: 'utf8' }).trim();
...
async function updatePackage(): Promise<void> {
    try {
        ...
        console.error(`${c.error('[ERROR]')} Update failed: ${(e as Error).message}`);
...
function parseArgs(args: string[]): ParsedArgs {
    const parsed: ParsedArgs = { command: 'start', options: {} };
