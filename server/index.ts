...
import { initializeDatabase, refreshTokensDb, sessionNamesDb, applyCustomSessionNames } from './database/db.js';
import { validateApiKey, authenticateToken, authenticateWebSocket } from './middleware/auth.js';
import { IS_PLATFORM } from './constants/config.js';
import { apiLimiter } from './middleware/rateLimiter.js';
import { errorHandler, notFoundHandler } from './middleware/errorHandler.js';
import { logger } from './utils/logger.js';
import { WORKSPACES_ROOT, validateWorkspacePath } from './routes/projects.js';
import { ptySessionsMap, stripAnsiSequences, normalizeDetectedUrl, extractUrlsFromText, shouldAutoOpenUrlFromOutput, SHELL_URL_PARSE_BUFFER_LIMIT, PTY_SESSION_TIMEOUT } from './pty-manager.js';
import type { AuthRequest } from './types/index.js';

// Extend WebSocket with isAlive for heartbeat
...
let projectsWatcher: import('chokidar').FSWatcher | null = null;
let projectsWatcherDebounceTimer: ReturnType<typeof setTimeout> | null = null;
let debounceTimer: ReturnType<typeof setTimeout> | null = null;
const connectedClients = new Set<ExtWebSocket>();
let isGetProjectsRunning = false; // Flag to prevent reentrant calls

// Broadcast progress to all connected WebSocket clients
function broadcastProgress(progress: Record<string, unknown>) {
    const message = JSON.stringify({
        type: 'loading_progress',
        ...progress
    });
    connectedClients.forEach((client: ExtWebSocket) => {
...
app.delete('/api/projects/:projectName/sessions/:sessionId', authenticateToken, async (req, res) => {
    try {
        ...
        sessionNamesDb.deleteName(String(sessionId), 'claude');
...
app.get('/api/projects/:projectName/file', authenticateToken, async (req, res) => {
    try {
        ...
        const resolved = path.isAbsolute(filePathStr)
            ? path.resolve(filePathStr)
            : path.resolve(projectRoot, filePathStr);
...
app.get('/api/projects/:projectName/files', authenticateToken, async (req, res) => {
    try {
        ...
        try {
            ...
            actualPath = String(req.params.projectName).replace(/-/g, '/');
        }
        ...
        console.error('[ERROR] File tree error:', errMsg);
...
function handleChatConnection(ws) {
    ...
    ws.on('message', async (message) => {
        try {
            ...
            const errMsg = (error instanceof Error ? error.message : String(error));
            console.error('[ERROR] Chat WebSocket error:', errMsg);
            writer.send({
                type: 'error',
                error: errMsg
            });
...
function handleShellConnection(ws) {
    ...
    ws.on('message', async (message) => {
        try {
            ...
            const errMsg = (error instanceof Error ? error.message : String(error));
            console.error('[ERROR] Shell WebSocket error:', errMsg);
    ...
    try {
        ...
        const storage = multer.diskStorage({
            destination: async (req, file, cb) => {
                const uploadDir = path.join(os.tmpdir(), 'claude-ui-uploads', String(authReq.user!.id));
...
async function getFileTree(dirPath: string, maxDepth = 3, currentDepth = 0, showHidden = true): Promise<FileSystemItem[]> {
    // Using fsPromises from import
    ...
    try {
        ...
        const tokenCleanupInterval = setInterval(() => {
          try {
            ...
            console.error('[ERROR] Failed to cleanup expired tokens:', (e instanceof Error ? e.message : String(e)));
          }
        }, 6 * 60 * 60 * 1000);
