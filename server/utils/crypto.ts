...
const DEV_JWT_SECRET: string = 'claude-ui-dev-secret-do-not-use-in-production';

// Derive encryption key from dedicated ENCRYPTION_KEY or JWT_SECRET (which is already required)
const ENCRYPTION_KEY_SOURCE: string | null = process.env.ENCRYPTION_KEY
...
export function encrypt(plaintext: string): string {
  ...
  const cipher: crypto.CipherGCM = crypto.createCipheriv(ALGORITHM, ENCRYPTION_KEY, iv) as crypto.CipherGCM;
 ...
 * @param encryptedData - Base64-encoded ciphertext from encrypt()
...
export function decrypt(encryptedData: string): string {
  ...
  const authTag: Buffer = Buffer.from(authTagBase64, 'base64');

  const decipher: crypto.DecipherGCM = crypto.createDecipheriv(ALGORITHM, ENCRYPTION_KEY, iv) as crypto.DecipherGCM;
...
export function hashApiKey(apiKey: string): string {
  return crypto.createHmac('sha256', ENCRYPTION_KEY_SOURCE as string).update(apiKey).digest('hex');
}
 ...
 */
export function hashApiKeyLegacy(apiKey: string): string {
...
export function hashRefreshToken(token: string): string {
  return crypto.createHmac('sha256', ENCRYPTION_KEY_SOURCE as string).update(`refresh:${token}`).digest('hex');
}
