...
const spawnFunction = (process.platform === 'win32' ? crossSpawn : spawn) as typeof spawn;
...
interface GeminiOptions {
  ...
  cwd?: string;
  resume?: boolean;
  toolsSettings?: GeminiToolsSettings;
...
interface GeminiChildProcess extends ChildProcess {
...
interface GeminiConfigFile {
  ...
  geminiProjects?: Record<string, { mcpServers?: Record<string, unknown> }>;
}

let activeGeminiProcesses: Map<string, GeminiChildProcess> = new Map(); // Track active processes by session ID

async function spawnGemini(command: string, options: GeminiOptions = {}, ws: WsWriter): Promise<void> {
    const { sessionId, projectPath, cwd, resume, toolsSettings, permissionMode, images } = options;
    let capturedSessionId: string | undefined = sessionId; // Track session ID throughout the process
    let sessionCreatedSent = false; // Track if we've already sent session-created event
    let assistantBlocks: AssistantBlock[] = []; // Accumulate the full response blocks including tools

    // Use tools settings passed from frontend, or defaults
    const settings: GeminiToolsSettings = toolsSettings || {
        allowedTools: [],
        disallowedTools: [],
    ...
    const cleanPath: string = (cwd || projectPath || process.cwd()).replace(/[^\x20-\x7E]/g, '').trim();
    const workingDir: string = cleanPath;
    ...
    if (images && images.length > 0) {
        try {
            ...
            if (tempImagePaths.length > 0 && command && command.trim()) {
                ...
                const modifiedCommand: string = command + imageNote;

                // Update the command in args
                const promptIndex: number = args.indexOf('--prompt');
    ...
    try {
        const geminiConfigPath: string = path.join(os.homedir(), '.gemini.json');
        let hasMcpServers = false;

        try {
            await fs.access(geminiConfigPath);
            const geminiConfigRaw: string = await fs.readFile(geminiConfigPath, 'utf8');
            const geminiConfig: GeminiConfigFile = JSON.parse(geminiConfigRaw);
    ...
    let modelToUse: string = options.model || 'gemini-2.5-flash';
    ...
    const geminiPath: string = process.env.GEMINI_PATH || 'gemini';
    console.log('Spawning Gemini CLI:', geminiPath, args.join(' '));
    console.log('Working directory:', workingDir);

    let spawnCmd: string = geminiPath;
    ...
    return new Promise<void>((resolve, reject) => {
        const geminiProcess: GeminiChildProcess = spawnFunction(spawnCmd, spawnArgs, {
            cwd: workingDir,
            stdio: ['pipe', 'pipe', 'pipe'],
            env: { ...process.env } // Inherit all environment variables
        }) as GeminiChildProcess;
        ...
        const processKey: string = capturedSessionId || sessionId || Date.now().toString();
        ...
        const timeoutMs: number = 120000; // 120 seconds for slower models
        let timeout: ReturnType<typeof setTimeout> | undefined;

        const startTimeout = (): void => {
            if (timeout) clearTimeout(timeout);
            timeout = setTimeout(() => {
                const socketSessionId: string = typeof ws.getSessionId === 'function' ? ws.getSessionId()! : (capturedSessionId || sessionId || processKey);
                ws.send({
                    type: 'gemini-error',
                ...
                onToolResult: (event) => {
                    if (capturedSessionId) {
                        if (assistantBlocks.length > 0) {
                            sessionManager.addMessage(capturedSessionId, 'assistant', JSON.stringify([...assistantBlocks]));
                            assistantBlocks = [];
                        }
                        sessionManager.addMessage(capturedSessionId, 'user', JSON.stringify([{
                            type: 'tool_result',
        ...
        geminiProcess.stdout!.on('data', (data: Buffer) => {
            ...
            if (responseHandler) {
                ...
                const socketSessionId: string | undefined = typeof ws.getSessionId === 'function' ? (ws.getSessionId() || undefined) : (capturedSessionId || sessionId);
                ws.send({
        ...
        geminiProcess.on('close', async (code: number | null) => {
            ...
            if (finalSessionId && assistantBlocks.length > 0) {
                sessionManager.addMessage(finalSessionId, 'assistant', JSON.stringify(assistantBlocks));
            }
            ...
            if (geminiProcess.tempImagePaths && geminiProcess.tempImagePaths.length > 0) {
                ...
                if (geminiProcess.tempDir) {
                    await fs.rm(geminiProcess.tempDir, { recursive: true, force: true }).catch((err: Error) => { });
                }
            }
        ...
        geminiProcess.on('error', (error: Error) => {
            ...
            const errorSessionId: string | undefined = typeof ws.getSessionId === 'function' ? (ws.getSessionId() || undefined) : finalSessionId;
            ws.send({
                type: 'gemini-error',
...
function abortGeminiSession(sessionId: string): boolean {
    let geminiProc: GeminiChildProcess | undefined = activeGeminiProcesses.get(sessionId);
    let processKey: string = sessionId;
