...
import type { ProjectSession, ContentBlock } from './types/index.js';
...
interface DiscoveredProject {
  ...
  taskmaster?: Record<string, unknown>;
}
        ...
        if (tasksData.tasks) {
          ...
          Object.values(tasksData).forEach((tagData) => {
            if (tagData && typeof tagData === 'object' && 'tasks' in tagData) {
              tasks = tasks.concat((tagData as { tasks: unknown[] }).tasks);
            }
          });
        }
...
async function getProjects(progressCallback: ((progress: Record<string, unknown>) => void) | null = null) {
  const codexSessionsIndexRef: { sessionsByProject?: Map<string, unknown[]> } = {};
  ...
  try {
    ...
    const manualProjectsCount = Object.entries(config)
      .filter(([name, cfg]: [string, Record<string, unknown>]) => cfg.manuallyAdded && !existingProjects.has(name))
      .length;
    ...
    totalProjects = Object.entries(config)
      .filter(([name, cfg]: [string, Record<string, unknown>]) => cfg.manuallyAdded)
      .length;
  }

  // Add manually configured projects that don't exist as folders yet
  for (const [projectName, projectConfig] of Object.entries(config) as [string, Record<string, unknown>][]) {
    if (!existingProjects.has(projectName) && projectConfig.manuallyAdded) {
      ...
      let actualProjectDir: string = (projectConfig.originalPath as string) || '';
      ...
      const project: DiscoveredProject = {
        name: projectName,
        path: actualProjectDir,
        displayName: (projectConfig.displayName as string) || await generateDisplayName(projectName, actualProjectDir),
  ...
  try {
    ...
    for (const sessionId of sessionDirs) {
      ...
      try {
        ...
        const sessionName = (metadata.title as string) || (metadata.sessionTitle as string) || 'Untitled Session';
...
async function getCodexSessions(projectPath: string, options: { limit?: number; indexRef?: { sessionsByProject?: Map<string, unknown[]> } | null } = {}) {
  ...
  for (const [sessionId, session] of sessionManager.sessions) {
    ...
    for (const msg of session.messages) {
      ...
      const text = typeof msg.content === 'string' ? msg.content
        : Array.isArray(msg.content) ? (msg.content as ContentBlock[]).filter(p => p.type === 'text').map(p => p.text).join(' ')
        : '';
      if (!text) continue;
